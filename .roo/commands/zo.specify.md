...
The text the user typed after `/zo.specify` in the triggering message **is** the feature description. Assume you always have it available in this conversation even if `$ARGUMENTS` appears literally below. Do not ask the user to repeat it unless they provided an empty command.
...
2. **Check for existing branches before creating new one**:
   ...
   **IMPORTANT**:
   ...
   - Match ALL branches/directories with the feature number pattern [0-9]{3}-*
...
3. Load `.zo/templates/spec-template.md` to understand required sections.
...
6. **Design System Integration (Optional)**:
   ...
   b. **Check for Global Design System**:
...
8. **Can be done later**: Design can always be added later with `/zo.design <feature> --design`
